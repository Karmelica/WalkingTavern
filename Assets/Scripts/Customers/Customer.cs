...
public class Customer : NetworkBehaviour, IInteractable
{
	...
	private void CustomerSetup()
	{
		...
		ears.ShowSelectedMesh(selectedEarsIndex.Value);
		shirtMesh.ShowSelectedMesh(selectedShirtIndex.Value);
		pantsMesh.ShowSelectedMesh(selectedPantsIndex.Value);
		hairMesh.ShowSelectedMesh(selectedHairIndex.Value);

		faceRenderer.sharedMaterials = new[] { new Material(faces[selectedFaceIndex.Value]) };
		foreach (var mesh in customerMesh) mesh.sharedMaterials = new[] { skins[selectedSkinIndex.Value] };
