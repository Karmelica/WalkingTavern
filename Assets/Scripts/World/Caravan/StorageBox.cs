...
namespace World.Caravan
{
	public class StorageBox : NetworkBehaviour, IInteractable
	{
		...
		{
			if (!IsServer) return;
			if (other.TryGetComponent(out FoodItem foodItem) && !foodItem.IsInteractedWith() &&
			    foodItem.ingredientType == ingredientBox) {
		...
		{
			if (unlimited) return $"\nStorage ({ingredientBox.ToString().SplitBigLetter()})";
			return $"\nStorage ({ingredientBox.ToString().SplitBigLetter()}: {_quantity.Value})";
		}
		...
		private void CheckIngredientServerRpc(RpcParams rpcParams = default)
		{
			...
			_quantity.Value = FoodStorage.Instance.GetIngredientCount(ingredientBox, unlimited);
			var i = Instantiate(Resources.Load<GameObject>("Prefabs/Food/Ingredients/" + ingredientBox),
				transform.position + transform.forward * 0.5f, Quaternion.identity);
