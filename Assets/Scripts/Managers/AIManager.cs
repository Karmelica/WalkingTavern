...
namespace Managers
{
	public class AIManager : NetworkBehaviour
	{
		...
		private readonly Queue<Transform> _availableSeats = new();
		private readonly Dictionary<DishType, int> _dishes = new();
		...
		{
			if (!_dishes.TryAdd(dish, 1)) _dishes[dish]++;
			UpdateGUI();
		}
		...
		private void UpdateGUI()
		{
			...
			if (_dishes.Count != 0) {
				foreach (var dish in _dishes)
					info += $"\n  {dish.Key.ToString().DeleteAndSplit("Dish")} x{dish.Value.ToString()}";
			}
		...
		private IEnumerator SpawnCustomer()
		{
			for (var i = 0; i < totalCustomers; i++) {
				var customerInstance = Instantiate(customerPrefab,
					spawnPoint.position + new Vector3(Random.Range(-5, 5), 0, Random.Range(-5, 5)),
					Quaternion.identity);
				...
				var rand = Random.Range(0, customer.ears.Length);
		...
		{
			if (firstNames.Count == 0 || secondNames.Count == 0) return "Noname";
