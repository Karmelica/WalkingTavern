...
namespace Player
{
    public class LobbyPreviewCharacter : MonoBehaviour
    {
        ...
        {
            foreach (var mesh in playerSkinMesh)
            {
                mesh.sharedMaterial = skinsMaterials[index];
            }
        }
        
        public void ChangeShirtColor(int selectedShirtColor)
        {
            foreach (var mesh in playerShirtMesh)
            {
                mesh.sharedMaterial = clothesMaterials[selectedShirtColor];
            }
        }

        public void ChangePantsColor(int selectedPantsColor)
        {
            foreach (var mesh in playerPantsMesh)
            {
                mesh.sharedMaterial = clothesMaterials[selectedPantsColor];
            }
        }
        
        public void ChangeHairColor(int selectedHairColor)
        {
            foreach (var mesh in playerHairMesh)
            {
                mesh.sharedMaterial = hairMaterials[selectedHairColor];
            }
        }
        ...
        {
            Utilis.ShowSelectedMesh(playerEarsMesh, index);
        }

        public void ChangeHair(int selectedHair)
        {
            Utilis.ShowSelectedMesh(playerHairMesh, selectedHair);
        }

        public void ChangeShirt(int selectedShirt)
        {
            Utilis.ShowSelectedMesh(playerShirtMesh, selectedShirt);
        }

        public void ChangePants(int selectedPants)
        {
            Utilis.ShowSelectedMesh(playerPantsMesh, selectedPants);
        }
