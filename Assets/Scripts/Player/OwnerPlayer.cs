...
namespace Player
{
	...
	public class OwnerPlayer : NetworkBehaviour, InputSystem_Actions.IPlayerActions
	{
		...
		[Range(0f, 90f)] [SerializeField] private float walkableAngle = 45f;
		...
		private bool _isCookingPreTutorialState;
		private bool _isDrivingPreTutorialState;
		...
		public override void OnNetworkSpawn()
		{
			...
			if (!IsOwner) {
				...
				if (_playerCamera == null) throw new Exception("Player camera not found");
			...
			if (!_isCooking) {
				...
				if (Physics.Raycast(_playerCamera.transform.position, _playerCamera.transform.forward, out var hitInfo,
					...
					if (hitInfo.collider.TryGetComponent(out PortalCamera portal)) {
						_windowMatrix = portal.otherPortal.transform.localToWorldMatrix *
				...
				_playerCamera.transform.position =
					Vector3.Lerp(_playerCamera.transform.position, _minigameCamera.position, 0.5f);
				_playerCamera.transform.rotation =
					Quaternion.Lerp(_playerCamera.transform.rotation, _minigameCamera.rotation, 0.5f);
			}
		}
		...
		private void Move()
		{
			_rigidbody.AddForce(-new Vector3(_rigidbody.linearVelocity.x, 0, _rigidbody.linearVelocity.z),
			...
			if (_canMove) {
				...
				if (_rigidbody.linearVelocity.magnitude < moveForce) {
					if (Physics.Raycast(transform.position + Vector3.up * 0.05f, moveVector + Vector3.up * -1,
						    out var hit, 0.4f)) {
						if (Vector3.Angle(hit.normal, Vector3.up) > walkableAngle &&
						    !_networkedPlayer.IsGrounded) {
							return;
						}
					}
			...
			if (_caravanControl) _caravanControl.Drive(_inputVector);
		}
		...
		{
			if (PlayerPrefs.GetInt("moving", 0) == 0) ShowTutorialPage("moving");
		...
		public void OnInteract(InputAction.CallbackContext context)
		{
			...
			if (GetHitInfo(out var interactable, out _, false) &&
			    (interactable.GetInteractText().Contains("Door") ||
			     interactable.GetInteractText().Contains("Storage"))) {
		...
		private bool GetHitInfo(out IInteractable interactableComponent, out IObjectID objectID,
			bool shouldCheckForInteracting = true,
			QueryTriggerInteraction triggerInteraction = QueryTriggerInteraction.Ignore)
		{
			...
			var rayHitInfo =
				Physics.RaycastAll(ray, InteractRange, LayerMask.GetMask("Interactable"), triggerInteraction);
		...
		{
			var hitObjects = Physics.RaycastAll(interactor.position, interactor.forward,
				3f, ~LayerMask.GetMask("Windows"), QueryTriggerInteraction.Ignore);
			...
			Physics.Raycast(interactor.position + interactor.forward, Vector3.down,
				out var groundHit, float.PositiveInfinity, ~LayerMask.GetMask("Ignore Raycast"),
		...
		private void ShowTutorialPage(string tutorialName)
		{
			...
			_isCookingPreTutorialState = _isCooking;
			_isDrivingPreTutorialState = _isDriving;
			SetCanMove(false);
		...
		private void CloseTutorialPage()
		{
			...
			SetCanMove(_canMovePreTutorialState);
			Cursor.lockState = _cursorPreTutorialState;
			Cursor.visible = _cursorVisiblePreTutorialState;
			_isCooking = _isCookingPreTutorialState;
			_isDriving = _isDrivingPreTutorialState;
		}
